...
def possible_bluff_and_situate():
    bluff = True
    discard = []
    card_list = []
    return bluff, discard, card_list
  
def our_decks(discard, bluff, card_list):
    main_deck = Deck()
    if bluff:
        card_list.append(discard)
    else:
        discard.append(card_list)
        
    return discard, card_list
    
def send_to_mid_deck(discard):
    for i in discard:
        print(i)
        
def send_to_detector(card_list):
    for i in card_list:
        print(i)
    
def main():
    bluff, discard, card_list = possible_bluff_and_situate()
    discard, card_list = our_decks(discard, bluff, card_list)
    send_to_mid_deck(discard)
    send_to_detector(card_list)
