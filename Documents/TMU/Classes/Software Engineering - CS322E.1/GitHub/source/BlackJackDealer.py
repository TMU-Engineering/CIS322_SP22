...
class BlackJackDealer:
    ...
    def Dealers_Moves(self):

        #   Display Dealer's Cards.
        print_hand = BlackJack.printCards(self)
        print(print_hand)

        #   Find the sum of the Dealer's hand.

        Sum = BlackJack.SumCards(self)
        #   Will be using this command later.
        #Sum = BlackJack.Hand_Value(dealer)

        #   Rule of the Dealer is that the Dealer
        #       always hits when their hand is below 16.
        while Sum < 17:
        #   Will be using this command later.
        #while BlackJack.Hand_Value(dealer) < 17:
            print('Dealer Hits')

            #   Dealer receives a card.
            BlackJack.addCard(self)

            #   Display Dealer's Cards.
            print_hand = BlackJack.printCards(self)
            print(print_hand)
    
            #   Find the sum of the Dealer's new hand.
            Sum = BlackJack.SumCards(self)
            #   Will be using this commanad later.
            #Sum = BlackJack.Hand_Value(dealer)

        #   If the sum is greater than 21 the dealer loses.
        #   Will be using this command later.
        if BlackJack.SumCards(self) > 21:
            #if BlackJack.Hand_Value(dealer) > 21:
            print('Dealer Busts')

        #   If the sum is greater than 16 but less than 21
        #       Dealer stands.
        else:
            print('Dealer stands')
